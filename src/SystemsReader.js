module.exports = (function () {
  ...
  const SHEET_FACTIONS = 3;
  const SHEET_SYSTEMS = 2;
  const SHEET_NEBULAE = 5;
	const SHEET_UNITS = 6;
   ...
   */
  const SystemsReader = function (logger) {
  ...
  SystemsReader.prototype.readSystems = function () {
    ...
    const systemsSheet = this.workbook[SHEET_SYSTEMS];
    //var nebulaeSheet = this.workbook[3];

    let curRow, curSystem, curAltNames, altRegexResult, curAffiliation, curScale;
    var parentheses;
    // sort out headers
  ...
  SystemsReader.prototype.readUnits = function () {
    this.readWorkbook();

    this.logger.log('Unit reader started');

    const unitsSheet = this.workbook[SHEET_UNITS];

    let curRow, curSystem, curUnit;
    // sort out headers
    const headerRowIdx = 1; // TODO magic number
    const columnIdxMap = {}; // map of column titles (lowercase) to column indices
    let curEra;

    this.units = [];

    curRow = unitsSheet.data[headerRowIdx];

    // column index map
    for (let i = 0, len = curRow.length; i < len; i++) {
      if (i < 8) { // TODO magic number
        columnIdxMap[(curRow[i] + '').toLowerCase()] = i;
      } else {
        columnIdxMap['era_' + (i - 8)] = i;
    ...
    for (let rowIdx = headerRowIdx + 1, endIdx = unitsSheet.data.length; rowIdx < endIdx; rowIdx++) {
      curRow = unitsSheet.data[rowIdx];
      ...
      // read system
      curSystem = this.systems.find((system) => system.sarnaLink === curRow[columnIdxMap['sarnalink']]);
      if (!curSystem) {
        console.warn(`Cannot find system with sarnaLink "${curRow[columnIdxMap['sarnaLink']]}"`);
        continue;
      }
      curSystem.units = curSystem.units || [];

      // era-specific unit
      for (let eraIdx = 0; eraIdx < this.eras.length; eraIdx++) {
        curEra = this.eras[eraIdx];
        curUnit = curRow[columnIdxMap['era_' + eraIdx]] || '';
        curSystem.units.push(curUnit);
      }
    }
  }
  ...
  SystemsReader.prototype.readLabelConfig = function () {
    ...
    const fileContent = fs.readFileSync(__dirname + '/../data/labelConfig.json', 'utf8');
    let arr;
    try {
